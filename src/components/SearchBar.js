...
export class SearchBar extends Component{
  ...
  autocomplete(){
      var z = window.document.getElementsByClassName("autocomplete-items");
      for(var i=0;i<z.length;i++){
        z[i].parentNode.removeChild(z[i]);
      }
      var val = this.state.value;
      var arr = this.state.countriesU;
      console.log("autocomplete");
      var parentNode = window.document.getElementById("myInput").parentNode;
      if(!val){
        return false;
      }
      ...
      for (var i = 0; i < arr.length; i++) {
        ...
        if (arr[i]["placeAddress"].substr(0, val.length).toUpperCase() == val.toUpperCase()) {
          /*create a DIV element for each matching element:*/
          console.log(arr[i]["placeAddress"]);
          var b = document.createElement("DIV");
          /*make the matching letters bold:*/
          b.innerHTML = "<strong>" + arr[i]["placeAddress"].substr(0, val.length) + "</strong>";
          b.innerHTML += arr[i]["placeAddress"].substr(val.length);
          /*insert a input field that will hold the current array item's value:*/
          b.innerHTML += "<input type='hidden' value='" + arr[i]["placeAddress"] + "' class = 'options-available'>";
          b.addEventListener("click", function(e) {
              /*insert the value for the autocomplete text field:*/
              parentNode.value = this.getElementsByTagName("input")[0].value;
  ...
  valueChange(){
    this.setState({
      value:window.document.getElementById("myInput").value
    });
  }
  componentDidUpdate(){
    console.log("shouldUpdates")
    ...
    return(
      <form autocomplete="off" action="/action_page.php">
        <div className="autocomplete">
          <input onChange={() => this.valueChange()} id="myInput" type="text" name="myCountry" placeholder="Enter the location" value={this.state.value}/>
        </div>
      </form>
    );
  }
